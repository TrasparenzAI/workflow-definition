...
printf "Try to create crawler_amministrazione_trasparente \n"
...
printf "\nTry to create crawler_result_failed\n"
...
printf "\nTry to create rule_detail_workflow\n"
...
printf "\nTry to create rule_detail_child_workflow\n"
...
printf "\nTry to create rule_detail_html_source_workflow\n"
( echo '['; cat rule_detail_html_source_workflow.json ; echo ']') | curl -X PUT $4/api/metadata/workflow -H "Authorization: Bearer $TOKEN" -H 'Content-type:application/json' --data @-
printf "\nTry to START workflow\n"
curl -X POST $4/api/workflow -H "Authorization: Bearer $TOKEN" -H 'Content-type:application/json' --data @start_amm_trasparente.json
printf "\n"
